try 
{
	url = "https://creator.zoho.com/appbuilder/service_recprotect/quotation/workflowbuilder/Update_Sync_Quote_To_CRM_1/edit";
	/*
...
*/
	...
	// // 		{
	// // 			thisapp.CRM.AdditionalNames_to_Lead_Sync_Create(customers.Customer_ID,"Trailer");
	// // 		}
	// 	}
	// --------------------- Sync Customer to CRM End------
	/*
	Note : Dont Change the below order based on the quote status
	*/
	//-----------Add Payment Transaction & DOcument Generation once Payment Completed--------------
	//-----------------Sync To CRM----------------
	thisapp.Developer.addActivityLog("Trailer Quote On Edit Success--" + input.ID,"Custom Function Called to Sync with CRM","Record ID-" + input.ID,"null");
	if(input.Quote_Status == "Payment Pending" || input.Quote_Status == "Completed" || input.Quote_Status == "Return To Payment" || input.Quote_Status == "In Progress" || input.Quote_Status == "Referral" || input.Quote_Status == "Saved" || input.Quote_Status == "Return To Broker")
	{
		...
		{
			thisapp.Developer.addActivityLog("Trailer Quote On Edit Success---" + input.ID,"Custom Function Called to Create the Combined Documents & Transaction history add","Record ID-" + input.ID,"null");
			thisapp.Generic_Functions.New_Buisness_Payment_Transaction(input.ID,"Trailer");
			//------------------Combine Document---------------
			//-------Callled this function after invoice & payment received creation ------
			// 			thisapp.Combine_Document.Trailer_Policy(input.ID);
		}
	}
	/*
	Note - End : Dont Change the below order based on the quote status
	*/
	//----------Upload Workdrive----------
	thisapp.Developer.addActivityLog("Trailer Quote On Edit Success---" + input.ID,"Custom Function Called to Upload the Detail Page Document to the Workdrive","Record ID-" + input.ID,"Workdrive Folder Link-" + input.NB_Workdrive_Folder_Link);
...
{
	thisapp.Developer.addDeveloperLog("Trailer Quote---" + input.ID,"Trailer Quote Edit Record OnSuccess",url,input.ID.toString(),e,"Creator");
}

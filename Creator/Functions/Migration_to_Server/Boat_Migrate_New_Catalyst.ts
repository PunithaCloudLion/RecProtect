void Migration_to_Server.Boat_Migrate_New_Catalyst()
{
	try 
	{
		fetchEndPoint = API_Configuration[Name_Process == "Boat Sync to Webapp - Update"].End_Point;
		orgString = thisapp.Server_Side.org_info();
		get_allboat = BoatQuote[ID == 4564627000002138143];
		//get_allboat=BoatQuote[Migrated == true && Migrated_to_Server != true] range from 0 to 50;
		// 		get_allboat = BoatQuote[Migrated == true] range from 851 to 900;
		count = 0;
		if(get_allboat.count() > 0)
		{
			for each  boatquote_data in get_allboat
			{
				...
				if(isvalid == true)
				{
					...
					for each  names in boatquote_data.Additional_Names
					{
						if(names.Additional_Insured_First_Name != null && names.Additional_Insured_Last_Name != null)
						{
							...
							{
								// 								info "Customer_ID is not Null";
								if(names.Customer_ID.Server_Customer_ID != null)
								{
									// 									info "Server_Customer_ID is not Null";
								...
								// 								info "Additional Mps--" + additional_names_map;
				...
				{
					quoteupdate.put("overrided_tax_percent",boatquote_data.Override_Tax.Tax);
				}
				...
				if(resp.isEmpty() == false)
				{
					if(resp.get("success") == "true")
					{
						...
						// 						info "Success";
						boatquote_data.Server_Migrated_Error_Resp="";
					}
					else
					{
				...
				// 				info boatquote_data.Quote_Record_ID_Server;
			}
			// 			info count;
		}
	}

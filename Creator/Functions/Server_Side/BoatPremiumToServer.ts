void Server_Side.BoatPremiumToServer(int boatID)
{
	try 
	{
		thisapp.Developer.addActivityLog("Server_Side.BoatPremiumToServer--" + boatID,"Function Call Start","ID","null");
		fetchBoat = Boat_Policy_change_Request[ID == boatID];
			...
			if(fetchBoat.Boat != null)
			{
				...
				for each  boatData in fetchBoat.Boat
				{
					...
					if(boatData.Admin_Override == "Yes")
					{
						...
						{
							boatsMap.put("when_suspended",boatData.When);
						}
					}
		...
		thisapp.Developer.addActivityLog("Server_Side.BoatPremiumToServer--" + boatID.tostring(),"Function Call Ended and Record updated in webapp ","ID---" + boatID,boatPostData.tostring());
	}
	catch (e)
	{
		thisapp.Developer.addDeveloperLog("BoatQuoteSync","Server_Side.BoatPremiumToServer:" + boatID.tostring(),"Record updated in webapp",boatID.tostring(),e,"creator");
	}
}
void Server_Side.TrailerPremiumToServer(int trailerID)
{
	try 
	{
		thisapp.Developer.addActivityLog("Server_Side.TrailerPremiumToServer--" + trailerID,"Function Call Start","ID","null");
		getTrailer = Trailer_Policy_Change_Request[ID == trailerID];
		dataMap = Map();
		customerMap = Map();
		...
		if(getCustomer.count() > 0)
		{
			...
			if(getTrailer.Trailer != null)
			{
				for each  trailer in getTrailer.Trailer
				{
					...
					trailerMap.put("taken_to_us_gt_90_days",trailer.Is_the_trailer_taken_into_the_USA_for_more_than_90_days);
		...
		thisapp.Developer.addActivityLog("Server_Side.TrailerPremiumToServer--" + trailerID.tostring(),"Function Call Ended and Record updated in webapp ","ID---" + trailerID,trailerPostData.tostring());
	}
	catch (e)
	{
		thisapp.Developer.addDeveloperLog("TrailerQuoteSync","Server_Side.TrailerPremiumToServer:" + trailerID.tostring(),"Record updated in webapp",trailerID.tostring(),e,"creator");
	}
}
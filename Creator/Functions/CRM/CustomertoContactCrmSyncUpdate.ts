void CRM.CustomertoContactCrmSyncUpdate(int customerID, int crmcontactid)
{
	try 
	{
		thisapp.Developer.addActivityLog("Update Contact(CRM.CustomertoContactCrmSyncUpdate)--" + customerID.toString(),"Update Contact in CRM","Funcation Call Start","");
		...
		if(fetchCustomers.count() > 0)
		{
			...
			if(crmcontactid != null)
			{
				...
				updateRes = zoho.crm.updateRecord("Contacts",crmcontactid,customerMap,{"trigger":{"workflow"}});
				if(fetchCustomers.Zoho_Crm_ID == null || fetchCustomers.Zoho_Crm_ID == "")
				{
					fetchCustomers.Zoho_Crm_ID=crmcontactid;
				}
				thisapp.Developer.addActivityLog("Update Contact(CRM.CustomertoContactCrmSyncUpdate)--" + customerID.toString(),"Update Contact API Call Response",customerMap.toString(),updateRes.toString());
			}
		}
		thisapp.Developer.addActivityLog("Update Contact(CRM.CustomertoContactCrmSyncUpdate)--" + customerID.toString(),"Update Contact in CRM","Funcation Call End","");
	}
	catch (e)
	{
		thisapp.Developer.addDeveloperLog("Customer","Update Contact(CRM.CustomertoContactCrmSyncUpdate)--" + customerID.toString(),"Update Contact Error Response","",e,"creator");
	}
}
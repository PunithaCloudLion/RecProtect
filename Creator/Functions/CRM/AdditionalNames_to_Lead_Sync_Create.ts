void CRM.AdditionalNames_to_Lead_Sync_Create(int cusID, string leadFor)
{
	try 
	{
		thisapp.Developer.addActivityLog("Create Additional Lead(CRM.AdditionalNames_to_Lead_Sync_Create)--" + cusID.toString(),"Create Additional Lead in CRM -- " + input.leadFor,"Funcation Call Start","");
		fetch_Cust = Customer[ID == input.cusID];
		if(fetch_Cust.count() > 0)
		{
			...
			// 			info fetch_Cust.Zoho_Crm_ID;
			if(fetch_Cust.Zoho_Crm_ID == "" || fetch_Cust.Zoho_Crm_ID == null)
			{
				createResp = zoho.crm.createRecord("Leads",dataMap,{"trigger":{"workflow"}});
				thisapp.Developer.addActivityLog("Create Additional Lead(CRM.AdditionalNames_to_Lead_Sync_Create)--" + cusID.toString(),"Create Additional Lead API Call Response ",dataMap.toString(),createResp.toString());
				fetch_Cust.Zoho_Crm_ID=createResp.get("id").tostring();
				fetch_Cust.Customer_Type="Lead";
			}
		}
		thisapp.Developer.addActivityLog("Create Additional Lead(CRM.AdditionalNames_to_Lead_Sync_Create)--" + cusID.toString(),"Create Additional Lead in CRM -- " + input.leadFor,"Funcation Call End","");
	}
	catch (e)
	{
		thisapp.Developer.addDeveloperLog("Customer","Create Additional Lead(CRM.AdditionalNames_to_Lead_Sync_Create)--" + cusID.toString(),"Create Additional Lead Error " + input.leadFor,"",e,"creator");
	}
}